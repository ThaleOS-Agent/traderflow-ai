...
export class BacktestEngine {
  ...
  async runBacktest(config = {}) {
    ...
    logger.info(`Starting backtest: ${backtestConfig.strategies.join(', ')} on ${backtestConfig.symbols.join(', ')}`);
    ...
    for (const pos of positions) {
      const lastPrice = equity[equity.length - 1]?.value || pos.entryPrice;
  ...
  async fetchHistoricalData(symbol, timeframe, startDate, endDate) {
    // Use Binance for historical data (most reliable)
    const connector = new MultiExchangeConnector('binance', true);
    
    try {
      const klines = await connector.getKlines(symbol, timeframe, 1000);
